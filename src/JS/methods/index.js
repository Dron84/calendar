...
import { FormationCaption } from "./formation";
import { collisionsCaption, directCollision } from "./collisions";
import { SHA } from "./SHA";
import { Fazi } from "./Fazi";
import { Loss } from "./Loss";
import { whitOutWealth } from "./whitOutWealth";
  ...
  const getBacziArr = (firstDay, countOfDays) =>
    60 - countOfDays > firstDay
      ...
      : [
          ...
          ...getBacziInRange(
            0,
            getCorrectionNumber(findDayFirstDayInMounth(date) + dayInMounth)
          )
        ];
...
export const getBacziYear = (i, date) => {
  const yearBegin = mounthBegin[date.year][1].begin_date.match(/(\d+)\.(\d+)/);
  const correction =
...
export const mounthCaption = (mounthArray, date) => {
  return mounthArray.filter(item => Number(item.id) === date.mounth)[0].name;
};
